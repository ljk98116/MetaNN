...
struct LayerInternalBuf_
{
    ...
    LayerTraits::LayerInternalBufType_<ElementType, DeviceType, CateTypeBatch>,
    LayerTraits::LayerInternalBufType_<ElementType, DeviceType, CateTypeSingle>
    >::type;
};
...
void MatrixGradCollect(const TWeight& weight, TGrad& grad, TGradCollector& col)
{
    while(!grad.empty())
    {
        auto g = grad.top();
